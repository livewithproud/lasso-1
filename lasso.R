...
lasso <- function(x , y, eps = .Machine$double.eps, max.steps = 20,trace = TRUE)
{
  call <- match.call()
  ### 参数
  nm <- dim(x)
  ...
  active <- NULL
  len.act <- matrix(0, max.steps +1, 1)
  ...
  object <- list(beta = beta, Cp = Cp, RSS = RSS, len.act = len.act)
  ### R2 = R2, A = A 
